...
- **提交者身份脱敏**：仓库原有 15 个提交的作者与提交者邮箱为 `2282781933@qq.com`（QQ 号可直接反查联系人），仓库转公开后会一并暴露。已用 `git filter-branch --env-filter` 将全部 20 个提交的邮箱改写为 GitHub noreply 地址 `83538532+zisjugoudan@users.noreply.github.com`——GitHub 仍按该地址把提交归属到项目负责人账号，仅去掉可联系的个人邮箱。改写前后 `git diff` 为空，提交内容与时间未变。改写前的历史保留在本地分支 `backup/pre-email-desensitize`（未推送）。仓库级 `user.email` 同步改为 noreply，后续提交不再写入 QQ 邮箱。受此影响，`tests/acceptance/OPS-003-github-publish-2026-07-28.md` 中引用的根提交 SHA `df55ef4` 已失效，新根提交为 `5d1e326`，该记录已追加修订说明。
